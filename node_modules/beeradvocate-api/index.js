...
exports.beerPage = function (url, callback) {
    ...
    request(url, function (error, response, html) {
        ...
        if (!error && response.statusCode == 200) {
            ...
            // Brewery details
            var links = $('#info_box').find('a');
            var brewery_state, brewery_country, beer_style;
            if (links.length > 4) {
            ...
            var stuff = $('#rating_fullview_content_2').each((review, data) => {
                var review = '';
                data.children.forEach((child, index) => {
                    if (child.type == 'text' && !child.data.toLowerCase().includes('rdev')) {
                        var myString = child.data;
                        myString = myString.replace(/(\r\n|\n|\r)/gm, ' ');
                        myString = myString.replace(/[^A-Za-z\s]/g, '');
                        myString = myString.replace(/nbsp/g, '');
                        myString = myString.toLowerCase();
                        review += ' ' + myString;
                    }
                });
                written_reviews.push(review);
...
exports.beerReviews = function (url, callback) {
    var url = "http://beeradvocate.com" + url;
    var written_reviews = [];

    var getPage = (start) => {
        request(url + "?start=" + start, (error, response, html) => {
            ...
            if (!error && response.statusCode == 200) {
                ...
                console.log(start);
                var stuff = $('#rating_fullview_content_2').each((index, data) => {
                    var review = '';
                ...
                if ($('#rating_fullview_content_2').length == 0) {
                    callback(written_reviews);
                } else {
                    getPage(start+25);
                }
